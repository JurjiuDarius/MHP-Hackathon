...
class TestMyService(unittest.TestCase):
    ...
    def test_create_booking(self):
        ...
        with patch('service.booking_service.Booking', mock_booking_class), \
            ...
            booking = create_booking(user_id=1, room_id=2, date='2024-03-16', start='09:00', end='10:00')

            self.assertIsNotNone(booking)
            self.assertEqual(booking.user_id, 1)
            self.assertEqual(booking.room_id, 2)
            self.assertEqual(booking.date, '2024-03-16')
            self.assertEqual(booking.start, '09:00')
            self.assertEqual(booking.end, '10:00')

    def test_get_booking(self):
        self.mock_booking_model.query.get.return_value = MagicMock(id=1, user_id=1, room_id=2, date='2024-03-16',
