...
from service.authentication_service import login
...
class TestLogin(unittest.TestCase):
    ...
    def test_login_incorrect_password(self):
        ...
        self.mock_user.serialize.return_value = {
            "id": 1,
            "email": "test@example.com",
            "role": "employee",
        }
        ...
        with patch(
            "service.authenticatin_service.User.query",
            return_value=self.mock_user_query,
        ):
            self.mock_user_query.filter_by.return_value.first.return_value = (
                self.mock_user
            )
