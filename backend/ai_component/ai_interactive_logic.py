...
def one_hot_encode_day(day_of_week_numeric):
    # Define the number of classes (days of the week)
    num_classes = 5
    # Create one-hot encoding
    one_hot = tf.one_hot(day_of_week_numeric, num_classes)
...
class ai_controller:
    def __init__(self):
        self.desks_morning = pickle.load(
            open("ai_component/models/desks-morning.pickle", "rb")
        )
        self.desks_evening = pickle.load(
            open("ai_component/models/desks-evening.pickle", "rb")
        )
        self.meeting_rooms_9_11 = tf.keras.models.load_model(
            "ai_component/models/meeting-rooms-9-11-0.57.keras"
        )
        self.meeting_rooms_11_13 = tf.keras.models.load_model(
            "ai_component/models/meeting-rooms-11-13-0.57.keras"
        )
        self.meeting_rooms_13_15 = tf.keras.models.load_model(
            "ai_component/models/meeting-rooms-13-15-0.62.keras"
        )
        self.meeting_rooms_15_17 = tf.keras.models.load_model(
            "ai_component/models/meeting-rooms-15-17-0.56.keras"
        )
        self.presences = tf.keras.models.load_model(
            "ai_component/models/presences-0.00.keras"
        )

    def get_desk_prediction_morning(self, desk, day):
        deskname = desk + " " + str(get_day_of_week_numeric(day))
        return self.desks_morning[deskname] / 13

    def get_desk_prediction_evening(self, desk, day):
        deskname = desk + " " + str(get_day_of_week_numeric(day))
        return self.desks_evening[deskname] / 13
    ...
    def get_presence_prediction(self, day):
        encoded_day = one_hot_encode_day(get_day_of_week_numeric(day))
        array = np.array([[encoded_day]])
        array = np.array([data_entry.flatten() for data_entry in array])
        prediction = self.presences.predict(array)
